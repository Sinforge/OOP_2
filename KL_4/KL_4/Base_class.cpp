...
void Base_class::set_connect(TYPE_SIGNAL p_signal, Base_class* p_object, TYPE_HANDLER p_ob_handler) {
	o_sh* p_value;
...
void Base_class::delete_connect(TYPE_SIGNAL p_signal, Base_class* p_object, TYPE_HANDLER p_ob_handler) {
	for (unsigned int i = 0; i < connects.size(); i++) {
		if (connects[i]->p_signal == p_signal && connects[i]->p_cl_base == p_object && connects[i]->p_handler == p_ob_handler) {
...
//Рассылка сигналов
void Base_class::emit_signal(TYPE_SIGNAL p_signal, string& s_command, TYPE_HANDLER handler) {
	...
	for (unsigned int i = 0; i < connects.size(); i++) {
		if (connects[i]->p_signal == p_signal && connects[i]->p_handler == handler) {
...
TYPE_HANDLER Base_class::getHandler() {
	...
	case 6:
		...
		break;
	default:return HANDLER(Screen::handler);
